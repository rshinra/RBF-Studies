...
def Build_A_Rect(b,c_v,e_v):               # Matrix H (Ax=b, Hc=g)
    A = np.zeros((np.size(e_v)+1,np.size(c_v)+1))       # Why is there a 1 here?  (Integral Boundary Condition)
    for j in range(np.size(c_v)):     # J is columns of A
        for i in range(np.size(e_v)):    # I is rows of A
            A[i,j]=Aij(b,e_v[i],c_v[j])
    for i in range(np.size(e_v)):
...
def u_hat(b,x,c_v,c_k):
    ...
    return y
def u_hat_merit(b,x,c_v,c_k):               # FOR THIS CASE ONLY u should be < 1
    good = 1;
    y = np.zeros(np.size(x))
    for i in range(np.size(x)):
        for j in range(np.size(c_v)):
...
betaspace = np.linspace(betamin,betamax,(betamax-betamin)*10+1)
    # "Evaluation points"
xe = np.linspace(xbound0,xbound1,101)
    # Plotting points
xp = np.linspace(xbound0,xbound1,501)
...
for i in range(np.size(betaspace)):
    beta = betaspace[i]    
    H = Build_A_Rect(beta,xc,xe)
    g = Build_b(np.size(xe))                       # Same here with the "2"
    # Boundary equations
    for j in range(np.size(xc)):                #  Boundary Condition 0, integral, i.e. all ck must add up to that
        H[np.size(xe),j]=np.sqrt(np.pi)/beta
    g[np.size(xc)]=1
    ...
    if (np.abs(ck[np.size(xc)]) > .000001):
      ...
      lam[i] = 0
    SumC[i] = sum(H[np.size(xe)]*ck)
    Errr[i] = error(beta,xe,xc,ck)
    if(Errr[i] < minerr):
        betafinal = beta
        ckfinal = ck
        lamfinal = lam[i]
        SCfinal = SumC[i]
        minerr = Errr[i]
#    print(u_hat_merit(beta,xc,xc,ck))
ans=f(xp)    
print(betafinal)
print(lamfinal)
print(SCfinal)
...
ax2 = plt.subplot(223)
ax3 = plt.subplot(122)
plot_f_error(ax3,betafinal,xe,xc,ckfinal)
plot_f(ax1,betafinal,xp,xc,ckfinal)
ax1.plot(xp,ans,'b')
